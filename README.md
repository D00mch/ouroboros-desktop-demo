...
[![Version 4.50.0-rc.3](https://img.shields.io/badge/version-4.50.0--rc.3-green.svg)](VERSION)
...
|---------|------|-------------|
| 4.50.0-rc.3 | 2026-04-24 | **fix(review): converge the post-rc.2 re-audit findings before the next prerelease.** `ouroboros/extensions_api.py::api_extension_manifest` now surfaces runtime `load_error` from the live extension SSOT instead of the stale discovery-only field, so `/api/extensions` and `/api/extensions/<skill>/manifest` stop disagreeing about a broken extension. `ouroboros/extension_loader.py` reloads extensions from a fresh staged import tree so same-second edits to `plugin.py` / relative imports do not silently keep serving stale code, and now fails closed if the extension tree contains a symlink that resolves outside the reviewed skill checkout. `ouroboros/onboarding_wizard.py::_WIZARD_HIDDEN_PROVIDER_DEFAULTS` is now empty — the wizard no longer silently wipes `OPENAI_BASE_URL`, `OPENAI_COMPATIBLE_API_KEY`, `OPENAI_COMPATIBLE_BASE_URL`, or `CLOUDRU_FOUNDATION_MODELS_BASE_URL` when the user re-runs onboarding, because all four are user-visible Settings fields that the wizard itself does not expose (`web/modules/onboarding_wizard.js` drops the same four keys from its save payload). `ouroboros/skill_loader.py::_bundled_skills_dir()` now falls back to the source checkout's real repo root when `ouroboros.config.REPO_DIR` points at the launcher-managed default, so bundled reference skills still appear in source/dev runs without manual env exports. `ouroboros/tools/registry.py::list_non_core_tools()` now includes live extension tools, matching the rest of the registry surfaces. `scripts/build_repo_bundle.py` now refuses to emit a packaged manifest without a resolved release tag, and the platform build scripts now verify that the matching `v$(cat VERSION)` tag exists AND points at `HEAD` before packaging proceeds (README "Release tag prerequisite" + `docs/ARCHITECTURE.md` §8.1 document the new requirement). Docs were also expanded to describe the new `data/state/skills/<name>/__extension_imports/<uuid>/skill/` staging surface created by `_stage_extension_import_tree` and brought back into sync with the managed `repo.bundle` bootstrap model (`docs/ARCHITECTURE.md`, `prompts/SYSTEM.md`). Targeted regression slices now cover the runtime/reporting fixes, bundled-skill fallback, build-bundle tag requirement, onboarding preservation, symlink confinement, and docs/changelog sync. |
